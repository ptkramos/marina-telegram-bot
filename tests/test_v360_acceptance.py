...
class TestV360Acceptance(unittest.TestCase):
    ...
    def test_control_languages_use_existing_cycle_and_preserve_legacy_fallback(self):
        with patch.object(self.memory.cycle_mgr, 'get_cycle_info',
            ...
            for language, heading in [('en', '[CONTROL RULES]'), ('pt-BR', '[REGRAS DE CONTROLE]')]:
                with patch.object(settings, 'LIVING_WORLD_ENABLED', True), patch.object(settings, 'PROMPT_CONTROL_LANGUAGE', language):
        ...
        with patch.object(settings, 'LIVING_WORLD_ENABLED', False):
            legacy = self.builder.build_system_prompt(now=self.now)
        self.assertIn('Marina Seltin', legacy)
        with self.db.get_connection() as conn:
